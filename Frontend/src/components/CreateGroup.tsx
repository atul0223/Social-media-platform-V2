  ...
  export default function CreateGroup() {
    ...
    const { setIsCreatingGroup,setLoading }:any =useContext(UserContext);
  ...
  const handleCreateGroup = async () => {
    ...
    try {
    await axios.post(`${BACKENDURL}/chat/creategroup`, formData, {
        withCredentials: true,
        headers: {
    ...
    return (
      <div>
        ...
        {pageOneOpen ? (
            ...
            <div className="col-span-2 w-full h-screen bg-neutral-200  ">
              <div className="flex p-4">
                ...
                />
                <div className="font-serif ml-3  ">Add group members</div>
              {selectedPeoples.length<2 ? <></>:<div className="w-full  items-center flex justify-end"><button className="bg-orange-600 px-3 py-2 rounded" onClick={()=>{setPageOneOpen(false)}}>Next</button></div>
  }</div>
                ...
                {searchData.map((item:any) => {
                  return (
                    <div key={item._id} className=" cursor-pointer select-none">
                      <div
                        className={`w-full rounded-2xl pl-3 h-20 hover:bg-neutral-100 flex  items-center mb-1 ${selectedPeoples.includes(item._id)?"bg-gray-400":""}` }
                        key={item._id}
                      ...
                      >
                        <div className="w-12 h-12 rounded-full">
                          ...
                          />:<FaCheck
                            className="md:w-12 md:h-12 rounded-full w-10 h-10 bg-orange-600 text-black p-3"
                          />}
                        </div>
                        <div className="ml-3">
          ...
          <div className="col-span-2 w-full h-screen bg-neutral-200  ">
                ...
                <img
                  ...
                  alt=""
                  className="w-8 h-8 hover:bg-gray-500 rounded    active:bg-gray-500 active:w-9 active:h-9"
                  onClick={() => setPageOneOpen(true)}
                />
                <div className="font-serif  ml-3 w-full h-full  ">Set Group details</div>
              {GroupName?<div className="w-full mt-1  items-center flex justify-end"><button className="bg-orange-600 px-3 py-2 cursor-pointer rounded"onClick={handleCreateGroup} >Create </button></div>:<> </>
  }
